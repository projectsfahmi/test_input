...
class _PageLoginState extends State<PageLogin> {
  ...
  doLogin(email, password) async {
    ...
    try {
      final _baseurl = 'https://api.fahmi-rifai.com/api-shareme/api/Authtimeout/tokenjson';
      ...
      if (response.statusCode == 200) {
        ...
        if (output['status'] == true) {
          ...
          String token = output['data']['token'];
  ...
  saveSession(String email,String token) async {
    ...
    Navigator.pushAndRemoveUntil(
      context,
      MaterialPageRoute(
        builder: (BuildContext context) => const MsEmployee(),
      ),
      (route) => false,
    );
  }
